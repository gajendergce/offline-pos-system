...
# Runs store data sync via Laravel artisan command.
# Usage:
#   ./scripts/offline/sync-store-data.sh [store_id]
...
echo "Running offline:sync-store-data for store_id=$OFFLINE_STORE_ID"
...
php artisan offline:sync-store-data "$OFFLINE_STORE_ID" --base-url="$OFFLINE_API_BASE_URL" --token="$OFFLINE_TOKEN"
'

echo "offline:sync-store-data completed."
